...
NO2_day_profile_Berlin = "berlin_data_aq/hourly_meanvalue_rel_by_station.csv"
...
no2_day_df = pd.read_csv(base_folder+NO2_day_profile_Berlin)

# commute_df = commute_df.dropna()

# # Step 1: Convert time strings to seconds for median calculation
# # seconds = pd.to_datetime(commute_df['departure_home'], format='%H:%M').dt.hour * 3600 + pd.to_datetime(commute_df['departure_home'], format='%H:%M').dt.minute * 60
# seconds = pd.to_datetime(commute_df['departure_work'], format='%H:%M').dt.hour * 3600 + pd.to_datetime(commute_df['departure_work'], format='%H:%M').dt.minute * 60

# # Step 2: Compute the median in seconds
# median_seconds = seconds.median()

# # Step 3: Convert median back to hh:mm format
# median_time = pd.to_datetime(median_seconds, unit='s').strftime('%H:%M')
...
# === Time-variant NO2 exposure (based on first station only) ===

print("Applying hourly NO2 variation factors (first station only)")

# Read first row as the representative hourly profile
hourly_profile = no2_day_df.iloc[0][3:27]

# If there’s a non-numeric column like 'station', drop it
if not np.issubdtype(hourly_profile.index.dtype, np.number):
    hourly_profile = hourly_profile.drop(labels=['station'], errors='ignore')

# Convert to numpy array of length 24
hourly_factors = hourly_profile.to_numpy(dtype=float)
# Sanity check
if len(hourly_factors) != 24:
    raise ValueError(f"Expected 24 hourly factors, got {len(hourly_factors)}")

# Define home and work hours
home_hours = list(range(0, 9)) + list(range(17, 24))
work_hours = list(range(9, 17))

# Calculate per-agent hourly exposure
daily_avg, daily_min, daily_max = [], [], []

for _, row in commute_df.iterrows():
    hourly_no2 = np.zeros(24)
    for h in range(24):
        if h in work_hours:
            hourly_no2[h] = row["NO2_work"] * hourly_factors[h]
        else:
            hourly_no2[h] = row["NO2_home"] * hourly_factors[h]
    daily_avg.append(np.mean(hourly_no2))
    daily_min.append(np.min(hourly_no2))
    daily_max.append(np.max(hourly_no2))

# Store results
commute_df["NO2_daily_avg"] = daily_avg
commute_df["NO2_daily_min"] = daily_min
commute_df["NO2_daily_max"] = daily_max

print("Added time-variant NO2 exposure (min, max, avg based on first station).")
...
commute_df.to_csv(base_folder+ "berlin_output/agent_locations_no2_v2_timevar.csv", index=False)

# Take a random sample every 10 rows
commute_samples_df = commute_df.sample(n=1000, random_state=42)
commute_samples_df.to_csv(base_folder+ "berlin_output/agent_locations_no2_v2_timevar_subset.csv", index=False)
print("... all finished")